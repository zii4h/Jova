...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void _openStageEditor() {
    showStageEditorSheet(
      ...
      onSave: (order, labels) {
        widget.onStagesChanged(order, labels);
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    final counts = <ApplicationStatus, int>{
      for (final status in ApplicationStatus.values)
        status: widget.applications.where((app) => app.status == status).length,
    };

    var visible = widget.applications;

    if (_filter != null) {
      visible = visible.where((app) => app.status == _filter).toList();
    }
    ...
    for (final status in widget.stageOrder) {
      final items = visible.where((app) => app.status == status).toList()
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'JOVA',
              style: FieldLog.brand(size: 26, weight: FontWeight.w700),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                border: OutlineInputBorder(
                  ...
                  borderSide: const BorderSide(color: FieldLog.border),
                ),
                ...
                focusedBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(FieldLog.radiusControl),
                  borderSide: const BorderSide(color: FieldLog.textPrimary),
                ),
              ),
            ),
            ...
            StatusFilterChips(
              selected: _filter,
              counts: counts,
              order: widget.stageOrder,
              labels: widget.stageLabels,
            ...
            // Edit stages - separate row
            Align(
              alignment: Alignment.centerRight,
              child: TextButton.icon(
                onPressed: _openStageEditor,
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      style: IconButton.styleFrom(
        ...
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(FieldLog.radiusControl),
          side: const BorderSide(color: FieldLog.border),
        ),
      ),
      icon: const Icon(
        Icons.add_rounded,
